...
/** Routes that require authentication. Public pages bypass all auth guards. */
...
export default function MainLayout({
  ...
  // Redirect unauthenticated users to login (auth-required routes only)
  useEffect(() => {
    if (requiresAuth && status === "unauthenticated") {
      router.push("/login");
    }
  }, [requiresAuth, status, router]);
  ...
  useEffect(() => {
    if (!requiresAuth || status !== "authenticated") return;
    const userRole = (session?.user?.role ?? "").toString().toUpperCase();
    if (userRole === "ADMIN" || userRole === "SUPER_ADMIN") {
      router.push("/admin/dashboard");
    }
  }, [requiresAuth, status, session, router]);
  ...
  }, [requiresAuth, status, session, pathname, router]);
  ...
  useNotificationSSE({ enabled: requiresAuth && status === "authenticated" });
