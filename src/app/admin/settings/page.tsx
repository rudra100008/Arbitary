"use client";

import React, { useEffect, useState } from "react";
import { toast } from "sonner";
import type { TogglePlatform } from "@/src/services/feature-flag.service";

type PlatformFlags = Record<TogglePlatform, boolean>;

const PLATFORM_META: Record<
  TogglePlatform,
  { label: string; description: string; color: string; icon: React.ReactNode }
> = {
  facebook: {
    label: "Facebook",
    description:
      "Facebook login, account linking, and Facebook tasks (like/comment verification).",
    color: "#1877F2",
    icon: (
      <svg
        className="w-5 h-5 text-white"
        fill="currentColor"
        viewBox="0 0 24 24"
      >
        <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
      </svg>
    ),
  },
  instagram: {
    label: "Instagram",
    description:
      "Instagram username linking and Instagram tasks (comment verification).",
    color: "#E1306C",
    icon: (
      <svg
        className="w-5 h-5 text-white"
        fill="currentColor"
        viewBox="0 0 24 24"
      >
        <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z" />
      </svg>
    ),
  },
};

const DEFAULT_FLAGS: PlatformFlags = { facebook: true, instagram: true };

export default function AdminSettingsPage() {
  const [flags, setFlags] = useState<PlatformFlags>(DEFAULT_FLAGS);
  const [loading, setLoading] = useState(true);
  const [savingKey, setSavingKey] = useState<TogglePlatform | null>(null);

  useEffect(() => {
    fetchFlags();
  }, []);

  const fetchFlags = async () => {
    setLoading(true);
    try {
      const res = await fetch("/api/admin/settings/feature-flags");
      if (!res.ok) throw new Error("Failed to load settings");
      const data = (await res.json()) as PlatformFlags;
      setFlags(data);
    } catch {
      toast.error("Failed to load platform settings");
    } finally {
      setLoading(false);
    }
  };

  const handleToggle = async (key: TogglePlatform) => {
    const nextEnabled = !flags[key];
    const label = PLATFORM_META[key].label;

    // Optimistic update — revert on failure
    setFlags((prev) => ({ ...prev, [key]: nextEnabled }));
    setSavingKey(key);
    try {
      const res = await fetch("/api/admin/settings/feature-flags", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ key, enabled: nextEnabled }),
      });
      if (!res.ok) {
        const data = await res.json().catch(() => ({}));
        throw new Error(data.error || "Failed to save");
      }
      toast.success(
        nextEnabled
          ? `${label} has been re-enabled`
          : `${label} has been disabled — existing users can no longer pick up, complete, or link ${label} until it's re-enabled`,
      );
    } catch (err: any) {
      setFlags((prev) => ({ ...prev, [key]: !nextEnabled }));
      toast.error(err.message || "Failed to save");
    } finally {
      setSavingKey(null);
    }
  };

  if (loading) {
    return (
      <div className="flex items-center justify-center py-32">
        <div className="w-8 h-8 border-4 border-black/10 border-t-black rounded-full animate-spin" />
      </div>
    );
  }

  return (
    <div className="animate-fade-in space-y-8 max-w-3xl">
      \
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-4 md:p-8 rounded-[2rem] border border-black/5 shadow-sm">
        <div>
          <h3 className="text-xl md:text-2xl font-black uppercase tracking-tighter">
            Settings
          </h3>
          <p className="text-[10px] md:text-xs font-bold text-zinc-400 uppercase tracking-widest mt-1">
            Platform feature flags
          </p>
        </div>
      </div>
      <div className="bg-white rounded-[2.5rem] border border-black/5 overflow-hidden shadow-sm p-4 md:p-8">
        <h4 className="text-sm font-black uppercase tracking-wider text-black mb-1">
          Social Integrations
        </h4>
        <p className="text-xs text-zinc-400 mb-6">
          Turning a platform off immediately blocks new logins, account linking,
          task pickup, and task completion for it across the whole app — no
          redeploy needed. Users who already completed tasks on that platform
          keep their points and history.
        </p>

        <div className="space-y-3">
          {(Object.keys(PLATFORM_META) as TogglePlatform[]).map((key) => {
            const meta = PLATFORM_META[key];
            const enabled = flags[key];
            const isSaving = savingKey === key;

            return (
              <div
                key={key}
                className="flex items-center justify-between px-4 py-4 bg-zinc-50 border border-black/5 rounded-2xl transition-all duration-200 hover:bg-white hover:shadow-sm"
              >
                <div className="flex items-center gap-3">
                  <div
                    className="w-10 h-10 rounded-xl flex items-center justify-center shadow-sm shrink-0"
                    style={{ backgroundColor: meta.color }}
                  >
                    {meta.icon}
                  </div>
                  <div>
                    <p className="text-sm font-black text-black uppercase tracking-wide">
                      {meta.label}
                    </p>
                    <p className="text-[11px] text-zinc-400 max-w-md">
                      {meta.description}
                    </p>
                  </div>
                </div>

                <div className="flex items-center gap-3 shrink-0">
                  <span
                    className={`text-[10px] font-bold px-2.5 py-1 rounded-full uppercase tracking-wider border ${
                      enabled
                        ? "bg-emerald-50 text-emerald-600 border-emerald-100"
                        : "bg-red-50 text-red-500 border-red-100"
                    }`}
                  >
                    {enabled ? "Enabled" : "Disabled"}
                  </span>
                  <button
                    role="switch"
                    aria-checked={enabled}
                    aria-label={`Toggle ${meta.label}`}
                    disabled={isSaving}
                    onClick={() => handleToggle(key)}
                    className={`relative w-12 h-6 rounded-full transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed ${
                      enabled ? "bg-black" : "bg-zinc-300"
                    }`}
                  >
                    <span
                      className={`absolute top-0.5 left-0.5 w-5 h-5 rounded-full bg-white shadow transition-transform duration-200 ${
                        enabled ? "translate-x-6" : "translate-x-0"
                      }`}
                    />
                  </button>
                </div>
              </div>
            );
          })}
        </div>
      </div>
    </div>
  );
}
