...
export default function AdminEvents() {
  ...
  const resetForm = () => {
    ...
    setAccessTypes([{ id: Math.random().toString(), title: "", price: "", pointCost: 0 }]);
    setContentSections([
      { id: Math.random().toString(), type: "content", content: "" },
    ]);
  };
  ...
  const handleDeleteEvent = async (id: string, title: string) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        toast.error(data.message || "Something went wrong while deleting. Please try again.");
      }
    } catch (e) {
      console.error(e);
      toast.error("Unable to delete the event. Please check your connection and try again.");
    } finally {
      setLoadingEventId(null);
    }
  };
  ...
  const handleSaveEvent = async () => {
    ...
    if (accessTypes.filter((a) => a.title.trim().length > 0).length === 0) {
      localErrors.accessTypes = "At least one access type with a title is required";
    }
    ...
    try {
      ...
      const processedSections = await Promise.all(
        unfilteredSections.map(async (section) => {
          ...
          if (section.type === "media" && section.mediaItems) {
            ...
            return { id, type: section.type, content: section.content, mediaItems: processedMediaItems };
          }
      ...
      if (!response.ok && response.status !== 400) {
        ...
        toast.error(statusMessages[response.status] || `Unexpected error (${response.status}). Please try again.`);
        return;
      }
        ...
        if (data.details && typeof data.details === "object") {
          ...
          (Object.entries(data.details) as [string, string[]][]).forEach(
            ([field, messages]) => {
              ...
              const match = field.match(/^(accessTypes|contentSections|timelineItems)\.(\d+)\.(.+)$/);
              if (match) {
                const [, arrayName, indexStr, subField] = match;
                const index = parseInt(indexStr, 10);
                let itemId: string | number | undefined;
                if (arrayName === "accessTypes") itemId = filteredAccessTypes[index]?.id;
                if (arrayName === "contentSections") itemId = unfilteredSections[index]?.id;
                if (arrayName === "timelineItems") itemId = filteredTimelines[index]?.id;
                if (itemId !== undefined) {
                  // Check for nested media items
                  const nestedMediaMatch = subField.match(/^mediaItems\.(\d+)\.(.+)$/);
                  if (nestedMediaMatch && arrayName === "contentSections") {
                    ...
                    if (mediaItem) {
                      errors[`${arrayName}.${itemId}.mediaItems.${mediaItem.id}.${mediaField}`] = errorMessage;
                    } else {
          ...
          toast.error("Please correct the highlighted validation errors in the form.");
          triggerScrollToError();
        } else {
          toast.error(data.message || data.error || "Something went wrong while saving. Please try again.");
        }
      }
    } catch {
      toast.error("Unable to reach the server. Please check your internet connection and try again.");
    } finally {
      setIsSaving(false);
    }
  };
